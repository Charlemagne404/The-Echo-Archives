...
export function renderQuickDetailsEditor(candidate) {
  ...
  return `
    <section class="maintainer-detail-section import-details-editor">
      ...
      <div class="maintainer-review-grid"><label class="maintainer-field"><span>Source genres / corrections</span><input name="categories" value="${escapeHtml((objective.categories || []).join(", "))}" placeholder="sci-fi, horror" /><small>Imported cards show useful mapped genres from this field. Verify or correct them against the official source before promotion; generic drama-only mappings are not treated as reviewed discovery tags.</small></label><label class="maintainer-field"><span>Discovery tags</span><input name="tags" value="${escapeHtml((objective.manualTags || candidate.preparedRecord?.tags || []).join(", "))}" placeholder="Sci-fi, Space, Survival" /><small>Use approved listener-discovery terms. Unknown terms are retained as review proposals and never enter public discovery automatically.</small></label><label class="maintainer-field"><span>Completion</span><select name="completionStatus">${[["unknown", "Unknown"], ["ongoing", "Ongoing"], ["finished", "Finished"]].map(([key, label]) => `<option value="${key}" ${completionStatus === key ? "selected" : ""}>${label}</option>`).join("")}</select></label></div>
      <label class="maintainer-field"><span>Tag exception rationale</span><textarea name="taxonomyExceptionRationale" rows="2" placeholder="Why this term would materially improve listener discovery, why no approved tag fits, and the supporting source.">${escapeHtml(objective.taxonomyExceptionRationale || "")}</textarea></label>
      <div class="maintainer-review-grid">
