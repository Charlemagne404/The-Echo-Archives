...
const DEFAULT_PLAUSIBLE_SCRIPT_SRC = "https://plausible.io/js/script.manual.js";
...
function renderAnalyticsScript(entry) {
  ...
  const configuredScriptSrc = process.env.PLAUSIBLE_SCRIPT_SRC;
  const plausibleScriptSrc = String(configuredScriptSrc === undefined ? DEFAULT_PLAUSIBLE_SCRIPT_SRC : configuredScriptSrc).trim();
...
function isAnalyticsEnabled(entry) {
  const plausibleDomain = String(process.env.PLAUSIBLE_DOMAIN || "").trim();
  const configuredScriptSrc = process.env.PLAUSIBLE_SCRIPT_SRC;
  const plausibleScriptSrc = String(configuredScriptSrc === undefined ? DEFAULT_PLAUSIBLE_SCRIPT_SRC : configuredScriptSrc).trim();
  return Boolean(plausibleDomain && plausibleScriptSrc && entry.includeAnalytics !== false);
}
...
function renderPage(entry, partials, versions, homeConfig, seoContext, submitPrerender) {
  ...
  const analyticsEnabled = isAnalyticsEnabled(entry);
  ...
  const bodySections = [
    ...
    `  <script src="/shared/archive-similarity.js?v=${versions.archiveSimilarity}"></script>`,
...
function createPrecacheUrlSet(_manifest, versions) {
  ...
  if (versions.archiveSimilarity) {
    precacheUrls.add(`/shared/archive-similarity.js?v=${versions.archiveSimilarity}`);
  }
  ...
  const cacheVersion = hashBuffer(
    Buffer.from(
      JSON.stringify({
        ...
        archiveSimilarity: versions.archiveSimilarity,
...
async function main() {
  ...
  const versions = {
    ...
    archiveSimilarity: hashFile("shared/archive-similarity.js"),
