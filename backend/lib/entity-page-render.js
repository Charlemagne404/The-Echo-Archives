...
function renderEntityCard(entity, shows) {
  const catalogue = orderedShows(entity, shows);
  return `<article class="entity-card" data-entity-id="${escapeHtml(entity.id)}" data-entity-type="${escapeHtml(entity.type)}" data-entity-show-count="${catalogue.length}" data-entity-names="${escapeHtml(JSON.stringify([entity.name, ...entity.aliases]))}"><a href="${entityPath(entity.id)}" data-discovery-entity-id="${escapeHtml(entity.id)}" data-discovery-entity-type="${escapeHtml(entity.type || "unknown")}" data-discovery-surface="entity_directory_card">${renderEntityArt(catalogue)}<div class="entity-card-copy"><div class="entity-card-head"><span class="entity-type">${TYPE_LABELS[entity.type]}</span><span class="entity-card-signal" aria-hidden="true"></span></div><h2>${escapeHtml(entity.name)}</h2><span class="entity-count"><strong>${catalogue.length}</strong> ${catalogue.length === 1 ? "show" : "shows"} in the archive <span class="entity-count-arrow">${renderArrowIcon()}</span></span></div></a></article>`;
}
...
function renderEntityShortcut(entity, catalogue, index) {
  ...
  return `<a class="home-hero-route entity-featured-route${index === 0 ? " is-primary" : ""}" href="${entityPath(entity.id)}" data-discovery-entity-id="${escapeHtml(entity.id)}" data-discovery-entity-type="${escapeHtml(entity.type || "unknown")}" data-discovery-surface="entity_directory_featured" aria-label="Open ${escapeHtml(entity.name)}, ${countLabel}"><span class="home-hero-route-kicker">${countLabel}</span><span class="home-hero-route-label">${escapeHtml(entity.name)}</span></a>`;
}
...
function stripDirectoryKicker(markup) {
  return markup.replace(/<span class="entity-directory-kicker">[^<]*<\/span>/g, "");
}
