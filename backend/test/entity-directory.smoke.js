...
for (const width of [1440, 1024, 768, 390, 320]) {
  test(`Creators directory and detail navigation work at ${width}px`, async () => {
    ...
    try {
      ...
      assert.equal((await page.locator(".entity-detail-status").innerText()).toLowerCase(), "source-backed connection");
      assert.equal(await page.locator("#entityRecommendationsTitle").count(), 0);
      assert.ok(await page.locator(".entity-show-role").count() > 0);
      assert.match(await page.locator("#entityShowsTitle").innerText(), /Audio dramas and fiction podcasts|Complete catalogue/i);
      assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1));
