...
Each candidate includes a score out of 100, metadata coverage, scored
dimensions, and factual `reasons`. A future UI can render the reasons directly
without asking an AI service to explain a recommendation.
...
- `length.avgEpisodeMinutes` (falling back to the median) supplies a small episode-length signal when both shows have a numeric value.
...
popularity, release state, and review status are not scored. This avoids
turning prose, lifecycle metadata, or sparse editorial coverage into invented
similarity claims.
...
anchor shows. It does not modify authored catalog files or generated output.
