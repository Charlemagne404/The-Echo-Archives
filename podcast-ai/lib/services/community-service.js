...
function createCommunityService({ store }) {
  ...
  function submitRating({ podcastId, rating, profileId, userAgent, source = "web" }) {
  ...
  function removeRating({ podcastId, profileId, userAgent, source = "web" }) {
    const podcast = store.getPodcast(podcastId);
    if (!podcast) {
      const error = new Error("Unknown podcast.");
      error.statusCode = 404;
